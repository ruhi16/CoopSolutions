<div>
    <div class="rounded-lg shadow-sm overflow-hidden">
        ...
        <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6 gap-4">
            <div>
                <h1 class="text-2xl font-bold text-gray-900"><span class="text-blue-700">Loan Scheme</span> Management
                </h1>
                ...
                <tbody>
                    @forelse($loanSchemes as $loanScheme)
                        <tr class="bg-white border-b hover:bg-gray-50">
                            ...
                            <td class="px-4 py-3 text-sm font-medium text-gray-900">{{ Str::limit($loanScheme->name, 30) }}
                            </td>
                            <td class="px-4 py-3 text-sm text-gray-700">{{ $loanScheme->name_short }}</td>
                            <td class="px-4 py-3 text-sm text-gray-700">
                                {{ $loanScheme->start_date ? $loanScheme->start_date->format('M d, Y') : 'N/A' }}</td>
                            <td class="px-4 py-3 text-sm text-gray-700">
                                {{ $loanScheme->end_date ? $loanScheme->end_date->format('M d, Y') : 'N/A' }}</td>
                            <td class="px-4 py-3 text-sm text-gray-700">
                                <span
                                    class="{{ $loanScheme->is_emi_enabled ? 'text-green-600 font-semibold' : 'text-gray-500' }}">
                            ...
                            <td class="px-4 py-3 text-sm text-gray-700">
                                <span
                                    class="{{ $loanScheme->is_active ? 'text-green-600 font-semibold' : 'text-gray-500' }}">
                                    {{ $loanScheme->is_active ? 'Yes' : 'No' }}
                                </span>
                            </td>
                            <td class="px-4 py-3 text-sm font-medium text-gray-700">
                                <button wire:click="edit({{ $loanScheme->id }})"
            ...
            <div class="bg-white rounded-lg shadow-xl p-6 w-full max-w-3xl mx-4 my-8">
                ...
                <form wire:submit.prevent="store">
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div>
                            <label for="name_short" class="block text-sm font-medium text-gray-700 mb-2">Short Name
                                *</label>
                            <input wire:model="name_short" type="text" id="name_short"
                                class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500"
                        ...
                        <div class="flex items-center">
                            <label class="flex items-center cursor-pointer">
                                <input wire:model="is_emi_enabled" type="checkbox"
