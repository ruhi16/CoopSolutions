...
class Ec17ShfundBankTransactionComp extends Component
{
    ...
    public function refresh(){
        $this->transactions = Ec17ShfundBankTransaction::with('member')->get();
    ...
    public function openModal($transactionId = null){
        if($transactionId){
            $transaction = Ec17ShfundBankTransaction::find($transactionId);
    ...
    public function saveTransaction(){
        $this->validate([
            ...
            'transactionIdField' => 'required|string|unique:ec17_shfund_bank_transactions,transaction_id,' . ($this->transactionId ?: 'NULL') . ',id',
        ...
        try{
            Ec17ShfundBankTransaction::updateOrCreate([
            ...
            $this->closeModal();
            $this->refresh();
            session()->flash('success', 'Bank Share Fund Transaction ' . ($this->transactionId ? 'Updated' : 'Created') . ' Successfully');
        }catch(\Exception $e){
    ...
    public function deleteTransaction(){
        if($this->deleteConfirmId){
            try{
                $transaction = Ec17ShfundBankTransaction::find($this->deleteConfirmId);
                $transaction->delete();

                $this->refresh();
                $this->cancelDelete();
                session()->flash('success', 'Bank Share Fund Transaction Deleted Successfully');
            }catch(\Exception $e){
                session()->flash('error', $e->getMessage());
                $this->cancelDelete();
            }
    ...
    {
        return view('livewire.ec17-shfund-bank-transaction-comp');
    }
}
