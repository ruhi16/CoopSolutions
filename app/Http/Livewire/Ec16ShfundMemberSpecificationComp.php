...
class Ec16ShfundMemberSpecificationComp extends Component
{
    ...
    public function refresh(){
        $this->specifications = Ec16ShfundMemberSpecification::all();
    ...
    public function openModal($specificationId = null){
        if($specificationId){
            $specification = Ec16ShfundMemberSpecification::find($specificationId);
    ...
    public function saveSpecification(){
        ...
        try{
            Ec16ShfundMemberSpecification::updateOrCreate([
            ...
            $this->closeModal();
            $this->refresh();
            session()->flash('success', 'Share Fund Member Specification ' . ($this->specificationId ? 'Updated' : 'Created') . ' Successfully');
        }catch(\Exception $e){
    ...
    public function deleteSpecification(){
        if($this->deleteConfirmId){
            try{
                $specification = Ec16ShfundMemberSpecification::find($this->deleteConfirmId);
                $specification->delete();

                $this->refresh();
                $this->cancelDelete();
                session()->flash('success', 'Share Fund Member Specification Deleted Successfully');
            }catch(\Exception $e){
                session()->flash('error', $e->getMessage());
                $this->cancelDelete();
            }
    ...
    {
        return view('livewire.ec16-shfund-member-specification-comp');
    }
}
